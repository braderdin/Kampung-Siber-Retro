...
// Refactored to use Presigned URL architecture for direct client uploads
...
// Start: Presigned URL Response Type
...
// End: Presigned URL Response Type
...
export default function AssetUploadZone({ className }: AssetUploadZoneProps) {
  ...
  // Start: Upload Handler - Two-step Presigned URL Architecture
  const handleUpload = useCallback(async (file: File) => {
    try {
      ...
      const presignedResponse = await fetch("/api/storage", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        // Start: Presigned URL Request - Only metadata, no file content
        body: JSON.stringify({
      ...
      // Parse response once
      const responseData = await presignedResponse.json() as PresignedUrlResponse;

      if (!presignedResponse.ok || !responseData.success || !responseData.data) {
        throw new Error(responseData.error || "Gagal mendapatkan URL ungahan");
      }
      ...
      // Step 2: Execute Direct Binary Upload to Cloudflare R2 using Presigned PUT URL
      ...
      if (!uploadResponse.ok) {
        throw new Error("Gagal mengunduh fail ke Cloudflare R2");
      }
